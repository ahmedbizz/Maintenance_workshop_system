...
namespace WorkShop.Controllers
{
    public class OfficerController : Controller
    {
        ...
        {
            var request = _unitOfWork.devices.FindAll("Product",
                "Department",
                "Technician",
                "MaintenanceCard", 
                "SparePartRequests.Items.Product")
                .FirstOrDefault(r => r.Id == Id &&
                r.MaintenanceCard.Status == MaintenanceStatus.ApprovedByEngineer.ToString());
        ...
        public async Task<IActionResult> ApproveByOfficer(int Id)
        {
            try
            {
                ...
                if (device == null || card == null || request == null)
                {
                ...
                var storeUser = _unitOfWork.users.FindAll("UserDepartments")
                    .Where(su => StoreAdmins.Any( SA => SA.Id == su.Id)).ToList();
